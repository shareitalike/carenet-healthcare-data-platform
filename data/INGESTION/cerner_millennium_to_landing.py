...
spark = SparkSession.builder.appName("CernerMillenniumMySQLToLanding").getOrCreate()

# Google Cloud Storage (GCS) Configuration
GCS_BUCKET = args.bucket
HOSPITAL_NAME = "cerner-millennium"
...
if secret_data:
    try:
        ...
        db_host = db_creds.get("host", "34.59.188.6")
    except Exception as e:
        print(f"Error parsing secret JSON: {str(e)}")
        db_user, db_pass, db_host = "myuser", "mypass", "34.59.188.6"
else:
    db_user, db_pass, db_host = "myuser", "mypass", "34.59.188.6"

# MySQL Configuration
MYSQL_CONFIG = {
    "url": f"jdbc:mysql://{db_host}:3306/cerner_millennium_db?useSSL=false&allowPublicKeyRetrieval=true",
...
def get_latest_watermark(table_name):
    try:
        query = f"""
            ...
            FROM `{BQ_AUDIT_TABLE}`
            WHERE tablename = '{table_name}' and data_source = "cerner_millennium_db"
        """
...
def extract_and_save_to_landing(table, load_type, watermark_col):
    try:
        ...
        audit_df = spark.createDataFrame([
            ("cerner_millennium_db", table, load_type, df.count(), datetime.datetime.now(), "SUCCESS")], 
...
for row in config_df.collect():
    if row["is_active"] == '1' and row["datasource"] == "cerner_millennium_db": 
