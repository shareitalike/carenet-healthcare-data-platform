...
spark = SparkSession.builder.appName("EpicClarityNorthToLanding").getOrCreate()

# Google Cloud Storage (GCS) Configuration
GCS_BUCKET = args.bucket
HOSPITAL_NAME = "epic-clarity-north"
...
# MySQL Configuration for Epic Clarity North Campus
MYSQL_CONFIG = {
    "url": f"jdbc:mysql://{db_host}:3306/epic_clarity_north_db?useSSL=false&allowPublicKeyRetrieval=true",
...
def save_logs_to_gcs():
    ...
    log_filename = f"pipeline_log_north_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}.json"
...
def get_latest_watermark(table_name):
    try:
        query = f"""
            ...
            FROM `{BQ_AUDIT_TABLE}`
            WHERE tablename = '{table_name}' and data_source = "epic_clarity_north_db"
        """
...
def extract_and_save_to_landing(table, load_type, watermark_col):
    try:
        ...
        audit_df = spark.createDataFrame([
            ("epic_clarity_north_db", table, load_type, df.count(), datetime.datetime.now(), "SUCCESS")], 
...
for row in config_df.collect():
    if row["is_active"] == '1' and row["datasource"] == "epic_clarity_north_db": 
